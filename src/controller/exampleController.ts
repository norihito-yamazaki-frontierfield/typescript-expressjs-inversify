...
import { interfaces, controller, httpGet, httpPost, httpPut, httpDelete, request, requestBody, response, requestParam } from "inversify-express-utils";
...
export class ExampleController implements interfaces.Controller {
    ...
    @httpGet("/:key")
    private async list(@requestParam("key") key: string, @response() res: express.Response) {
        try {
    ...
    private async create(@request() req: express.Request, @response() res: express.Response) {
        ...
        try {
            ...
            res.status(201).send("Message set successfully.");
        } catch (error) {
            res.status(500).send("An error occurred while setting the message.");
        }
    }

    @httpPut("/:key")
    private async update(@requestParam("key") key: string, @requestBody() body: { value: string }, @response() res: express.Response) {
        const { value } = body;
        if (!value) {
